...
Function List-And-Manage-Jobs {
    try {
        $jobs = Read-Jobs
        if ($jobs.Count -eq 0) {
            Write-Host "No scheduled timers found." -ForegroundColor Yellow
            return
        }
        ...
        if ($index -ge 0 -and $index -lt $jobs.Count) {
            ...
            Unregister-ScheduledTask -TaskName $jobToDelete.JobName -Confirm:$false -ErrorAction SilentlyContinue
            
            $jobs = $jobs | Where-Object { $_.JobName -ne $jobToDelete.JobName }
            Write-Jobs -Jobs $jobs
            Write-Log -Message "Deleted scheduled job '$($jobToDelete.JobName)'."
            
            Write-Host "Scheduled job '$($jobToDelete.JobName)' has been deleted." -ForegroundColor Green
        } else {
...
Function Search-And-Delete-Job {
    try {
        ...
        [int]$index = $choice - 1
        if ($index -ge 0 -and $index -lt $matchingTasks.Count) {
            $taskToDelete = $matchingTasks[$index]
            
            Unregister-ScheduledTask -TaskName $taskToDelete.TaskName -Confirm:$false -ErrorAction SilentlyContinue
            
            Write-Host "Scheduled task '$($taskToDelete.TaskName)' has been deleted." -ForegroundColor Green
            ...
            if ($updatedJobs.Count -lt $jobs.Count) {
                ...
                Write-Host "Removed '$($taskToDelete.TaskName)' from local tracking file." -ForegroundColor Green
